...
class Helper {
  ...
  static async waitWithTimeout2(promise, altPromise, taskName, timeout) {
    let reject;
    const timeoutError = new TimeoutError(
      `waiting for ${taskName} failed: timeout ${timeout}ms exceeded`
    );
    const timeoutPromise = new Promise((_, x) => (reject = x));
    let timeoutTimer = null;
    ...
    try {
      ...
      if (er instanceof TimeoutError) {
        console.log(`_______ timeout occurred - running the second promise & retrying...`);
        await altPromise();
        const nextAttempt = await this.waitWithTimeout2(promise, altPromise, taskName, timeout);
        return nextAttempt;
      }
      throw (er);
    } finally {
